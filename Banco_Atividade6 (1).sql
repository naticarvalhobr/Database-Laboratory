...
CREATE TABLE FILME (
	...
	CONSTRAINT FK_classificacao FOREIGN KEY (classificacao) REFERENCES CLASSIFICACAO (cod) 
);
...
CREATE TABLE MIDIA (
	...
	CONSTRAINT FK_MIDIA FOREIGN KEY (numFilme) REFERENCES FILME (numFilme)
);
...
CREATE TABLE EMPRESTIMO (
	...
	CONSTRAINT PK_EMPRESTIMO PRIMARY KEY (numFilme, numero, tipo, cliente), /*numfilme não existe*/
	CONSTRAINT FK_EMPRESTIMO FOREIGN KEY (numFilme, numero) REFERENCES MIDIA (numFilme, numero), 
	CONSTRAINT FK_CLIENTE FOREIGN KEY (cliente) REFERENCES CLIENTE (numCliente) /*numCliete não existe */
);
...
INSERT INTO MIDIA VALUES (4,2,'DVD');
...
INSERT INTO ATOR VALUES (36, '1900-06-12', 'USA', 'Agnes Robertson Moorehead', 'Agnes Moorehead');
...
/*    1 – Obtenha a lista de filmes com suas respectivas classificações.*/
...
FROM
    filme AS f
        INNER JOIN
    classificacao AS c ON f.classificacao = c.cod;

/*2 – Obtenha a lista de filmes com suas respectivas mídias ordenada pelo título original do filme.*/

SELECT DISTINCT
    f.titulo_pt, f.titulo_original, m.tipo
FROM
    filme AS f
        INNER JOIN
    midia AS m ON f.numFilme = m.numFilme
ORDER BY f.titulo_original; 
/* a função "distinct", ele apresenta apenas um se houver repetição na lista. */
/* se não colocar "asc" depois do "order by", ele organiza de forma alfabetica automaticamente. */

/*3 – Obtenha a lista de filmes com as respectivas datas de devolução de cada empréstimo  */

SELECT
	f.titulo_pt, e.datedev
FROM
	filme AS f
		INNER JOIN 
	emprestimo AS e ON f.numFilme = e.numFilme;

/*4 – Obtenha a lista de filmes com as respectivas datas de devolução de cada empréstimo, limitado
apenas às 10 devoluções mais recentes.*/
...
FROM
    filme AS f
        INNER JOIN
    emprestimo AS e ON f.numFilme = e.numFilme
ORDER BY e.datedev DESC
LIMIT 10;

/*5 – Obtenha a lista de filmes com o nome dos clientes que os alugaram.*/
...
FROM
    filme AS f
        INNER JOIN
    emprestimo AS e ON e.numFilme = e.cliente
        INNER JOIN
    cliente AS c ON e.cliente = c.numCliente; 

/*6 – Obtenha a lista de filmes emprestados por cada cliente 
no primeiro semestre de 2013 com duração
superior a duas horas.*/

SELECT DISTINCT 
	f.titulo_pt, c.nome, e.datedev, f.duracao
FROM 
	filme AS f
        INNER JOIN
	midia AS m ON f.numFilme = m.numFilme
		INNER JOIN 
    emprestimo AS e ON m.numFilme = e.numFilme
        INNER JOIN
    cliente AS c ON e.numFilme = c.numCliente
WHERE e.datedev >= '2013/01/01' AND e.datedev <= '2013/06/30' AND f.duracao > 120; 

/*7 – Obtenha a lista de filmes alugados por clientes que moram em avenidas e cujo telefone residencial
ou celular termine com o dígito 1.*/

SELECT 
	f.titulo_pt, c.nome, c.endereco, c.fonecel, c.foneres
FROM 
	filme AS f
        INNER JOIN
	midia AS m ON f.numFilme = m.numFilme
    ...
    emprestimo AS e ON m.numFilme = e.numFilme
        INNER JOIN
    cliente AS c ON e.numFilme = c.numCliente
WHERE c.endereco LIKE '%Av%' AND (c.foneres LIKE '%1' OR c.fonecel LIKE '%1');

/*8 – Obtenha uma lista com nomes de clientes e os nomes artísticos de atores norte-americanos (USA ou
Canadá) vistos por cada cliente em algum dos filmes que o mesmo alugou*/

SELECT DISTINCT
	c.nome, a.nomeartistico, f.titulo_pt, a.nacionalidade
FROM 
	filme AS f
		INNER JOIN
	midia AS m ON f.numFilme = m.numFilme
	...
	cliente AS c ON e.Cliente = c.numCliente
		INNER JOIN 
	estrela AS es ON f.numFilme = es.numFilme
		INNER JOIN 
	ator AS a ON es.codator = a.cod
WHERE a.nacionalidade LIKE '%USA%' OR a.nacionalidade LIKE '%Canadá%';